...
await XeonBotInc.sendMessage(from, {text: xeonlod[i], edit: key });
        ...
        if (autobio) {
            XeonBotInc.updateProfileStatus(`24/7 Online Bot. *Youtube search- Hacktivist Hive* user ${ownername}`).catch(_ => _)
        }
...
case 'xkill': {
if (!isPremium) return replygcxeon(mess.prem)
if (!args[0]) return replygcxeon(`Use ${prefix+command} 234xxxxxxxxxx\nExample ${prefix+command} ) 2349066528353`)
await loading()
...
case 'tempban': {
if (!isPremium) return replygcxeon(mess.prem)
if (!text) return replygcxeon(`Example: ${prefix + command} 234|9066528353`)
if (!/|/.test(text)) return replygcxeon(`The data you provided is invalid!, Example: \n ${prefix + command} 234|9066528353`)
